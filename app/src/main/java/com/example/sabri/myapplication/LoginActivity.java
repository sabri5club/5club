...
// check if the Stored password matches with Password entered by user
        if(password.equals(storedPassword))
        {
            ...
            Intent troisiemeActivite = new Intent(LoginActivity.this,Menu.class);
        ...
        {
            Toast.makeText(LoginActivity.this, "User Name or Password does not create_match", Toast.LENGTH_LONG).show();
        }
    ...
    public boolean validate() {
        ...
        if (password.isEmpty() || password.length() < 4 || password.length() > 10) {
            _passwordText.setError("Entre 4 et 10 caracteres");
            valid = false;
        } else {
